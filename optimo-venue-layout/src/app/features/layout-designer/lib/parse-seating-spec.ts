/**
 * Client-side seating spec parser (same rules as edge function).
 */
...
export function parseSeatingSpecFromText(lines: string[]): {
  ...
  const rowsMatch =
    lower.match(/rows?\s*[:=\-]?\s*(\d+)/) ?? lower.match(/(\d+)\s*rows?/);
  if (rowsMatch) {
  ...
  if (!hasRows && !hasSeats && !hasSides) {
    return { spec: null, notes: ['Could not parse seating spec from text.'] };
  }
...
export function parseSeatingSpecFromOcrTokens(tokens: OcrTokenLike[]): {
  spec: SeatingChartSpec | null;
  notes: string[];
} {
  return parseSeatingSpecFromText(tokensToLines(tokens));
}

export function parseSeatingSpecFromRawText(text: string): {
  spec: SeatingChartSpec | null;
  notes: string[];
} {
  return parseSeatingSpecFromText(
